page 60149 FBM_ItemsBySite_CO
{
    Caption = 'Items by Site';
    ...
    layout
    {
        area(content)
        {
            ...
            part(MatrixForm; FBM_ItemsBySiteMatrix)
            {
                ApplicationArea = Location;
                ShowFilter = false;
            }
    ...
    begin
        ...
        if ShowColumnName then
            CaptionFieldNo := TempMatrixLocation.FieldNo("Site Name_FF")
        else
            CaptionFieldNo := TempMatrixLocation.FieldNo("Site Code");
    ...
    local procedure OnAfterSetTempMatrixLocationFilters(var TempMatrixLocation: Record FBM_CustomerSite_C temporary);
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnSetColumnsOnBeforeUpdateMatrixSubform(MATRIXCaptionSet: array[32] of Text[80]; var Matrix_Records: array[32] of Record FBM_CustomerSite_C; TempMatrixLocation: Record FBM_CustomerSite_C temporary; CurrSetLength: Integer);
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnSetColumnsOnAfterSetTempMatrixLocationFilters(var TempMatrixLocation: Record FBM_CustomerSite_C temporary)
    begin
    end;
}
