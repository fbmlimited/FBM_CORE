page 60150 FBM_ItemsBySiteMatrix
{
    Caption = 'Items by Site Matrix';
    Editable = false;
    LinksAllowed = false;
    PageType = ListPart;
    SourceTable = Item;
                ...
                group("&Item Availability by")
                {
                    ...
                    action(Period)
                    {
                        ...
                        RunPageLink = "No." = FIELD("No."),
                                      ...
                                      FBM_SiteFilter = FIELD(FBM_SiteFilter),
                    ...
                    action(Site)
                    {
                        ...
                        Caption = 'Location';
                        Image = Warehouse;
                        RunObject = Page FBM_ItemAvBySite;
                        RunPageLink = "No." = FIELD("No."),
                                      "Global Dimension 1 Filter" = FIELD("Global Dimension 1 Filter"),
                                      "Global Dimension 2 Filter" = FIELD("Global Dimension 2 Filter"),
                                      FBM_SiteFilter = FIELD(FBM_SiteFilter),
    ...
    var
        ...
        MatrixRecords: array[32] of Record FBM_CustomerSite_C;
        TempMatrixLocation: Record FBM_CustomerSite_C temporary;
        ItemAvailFormsMgt: Codeunit FBM_ItemAvFormsMgt_CO;
    ...
    begin
        TempItem.Copy(Rec);
        TempItem.SetRange(FBM_SiteFilter, MatrixRecords[ColumnID]."Site Code");
        TempItem.CalcFields(FBM_Inventory_FF);
        MATRIX_CellData[ColumnID] := TempItem.FBM_Inventory_FF;
        SetVisible();

        OnAfterMATRIX_OnAfterGetRecord(MATRIX_CellData, MatrixRecords, TempItem, ColumnID);
    end;

    procedure Load(MatrixColumns1: array[32] of Text[1024]; var MatrixRecords1: array[32] of Record FBM_CustomerSite_C; var MatrixRecord1: Record FBM_CustomerSite_C; CurrSetLength: Integer)
    begin
    ...
    begin
        ItemLedgerEntry.SetCurrentKey(
          "Item No.", "Entry Type", "Variant Code", "Drop Shipment", fbm_site, "Posting Date");
        ItemLedgerEntry.SetRange("Item No.", rec."No.");
        ItemLedgerEntry.SetRange(FBM_SITE, MatrixRecords[ColumnID]."Site Code");
    ...
    local procedure OnAfterMATRIX_OnAfterGetRecord(var MATRIX_CellData: array[32] of Decimal; var MatrixRecords: array[32] of Record FBM_CustomerSite_C; var TempItem: Record Item temporary; ColumnID: Integer)
    begin
    end;
