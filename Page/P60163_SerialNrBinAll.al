...
page 60163 FBM_SerialNrBinAll_CO
{
    ApplicationArea = Warehouse;
    Caption = 'Serial Nr & Bin - All';
    DeleteAllowed = false;
    Editable = false;
    InsertAllowed = false;
    ModifyAllowed = false;
    ...
    trigger OnOpenPage()
    begin
        //we.setfilter("Serial No.", '<>%1', '');
        we.FindFirst();
        repeat

            rec.setrange("Item No.", we."Item No.");
            rec.SetRange("Bin Code", we."Bin Code");
            rec.SetRange("Location Code", we."Location Code");
            rec.SetRange("Serial No.", we."Serial No.");
            if rec.FindFirst() then begin
                item.get(we."Item No.");
                rec.Cubage := item."Unit Cost";
                rec.Description := item.Description;
                rec.Quantity += we.Quantity;
                rec.Modify();
            end
            else begin
                rec.init;
                rec.TransferFields(we, true);
